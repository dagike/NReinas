	...
	public static ArrayList<Cromosoma> cruza(int numeroReinas,
		...
		for (int i = 0; i < numeroCruzas; i = i + 2) {
			...
			for (int j = punto2Cruza; j < numeroReinas; j++) {
				for (int j2 = punto1Cruza; j2 < punto2Cruza; j2++) {
					if ((poblacionCruza.get(i).getElementos().get(j).getPosicionX() != temp1.get(j2).getPosicionX())
							&& (poblacionCruza.get(i).getElementos().get(j).getPosicionY() != temp1.get(j2).getPosicionY())) {
						band++;
					}
					if ((poblacionCruza.get(i + 1).getElementos().get(j).getPosicionX() != temp2.get(j2).getPosicionX())
							&& (poblacionCruza.get(i + 1).getElementos().get(j).getPosicionY() != temp2.get(j2).getPosicionY())) {
						band2++;
					}
				}
				if (band == (punto2Cruza - punto1Cruza)) {
			...
			poblacionCruza.get(i).setElementos((ArrayList<Reina>) temp1.clone());
			poblacionCruza.get(i + 1).setElementos((ArrayList<Reina>) temp2.clone());
